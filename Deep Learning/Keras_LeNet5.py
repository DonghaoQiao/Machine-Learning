...
class LeNet(models.Sequential):
    def __init__(self, input_shape, n_classes):
        super().__init__()
        # layer 1, conv layer
        self.add(layers.Conv2D(filters=6, kernel_size=5, strides=1, activation='relu', input_shape=input_shape, padding='same'))
        # layer 2, pooling layer
        self.add(layers.AveragePooling2D(pool_size=2, strides=1, padding='valid'))
        # layer 3, conv layer
        self.add(layers.Conv2D(filters=16, kernel_size=5, strides=1, activation='relu', padding='valid'))
